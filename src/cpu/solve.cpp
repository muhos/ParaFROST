...
Solver::Solver(const string& _path) : formula(_path), sp(NULL), vsids(VSIDS_CMP(activity)), vschedule(SCORS_CMP(this)), bumped(0), conflict(NOREF), ignore(NOREF), cnfstate(UNSOLVED_M), intr(false), stable(false), probed(false), incremental(false), mapped(false), simpstate(AWAKEN_SUCC) {
