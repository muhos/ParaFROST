...
public:
  QUEUE() : _bumped(0), _first(0), _last(0), _free(0) {}
  ...
  __forceinline void toFront(const uint32& v) {
    CHECKVAR(v);
    outQue(v), inQue(v);
  }
  __forceinline uint32 previous(const uint32& v) {
    CHECKVAR(v);
    return links[v].prev;
  }
  __forceinline uint32 next(const uint32& v) {
    CHECKVAR(v);
    return links[v].next;
  }
