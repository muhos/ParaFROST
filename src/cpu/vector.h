...
class Vec {
  ...
  bool check(const S& idx) const {
    if (idx >= sz) {
      ...
      std::cerr << "ERROR - index is out of vector boundary (type: " << typeid(T).name() << ", index: " << (long long)idx << ", size:" << (long long)sz << ")" << std::endl;
...
public:
  ...
  __forceinline const T& operator[](const S& index) const {
    assert(check(index));
    return _mem[index];
  }
  __forceinline T& operator[](const S& index) {
    assert(check(index));
    return _mem[index];
  }
  ...
  __forceinline void insert(const T& val) {
    assert(cap > sz);
    _mem[sz++] = val;
  }
  ...
  __forceinline void reserve(const S& min_cap, const S& size) {
    reserve(min_cap);
    sz = size;
  }
  __forceinline void init(const S& off, const S& n, const T& val) {
    if (!val && !off) {
      std::memset(_mem, 0, n * sizeof(T));
    } else {
      for (S i = off; i < n; i++) _mem[i] = val;
    }
  }
