...
void rSort(T* d, const SZ& sz) {
  ...
  if (sz <= RSORT_THR) {
    ...
    radixSort(d, d + sz, DEFAULT_RANK<T>());
}
...
void rSort(Vec<T>& d, CMP cmp, RANK rank) {
  ...
  if (size <= RSORT_THR) {
    if (size <= INSORT_THR)
      insertionSort(d.data(), size, cmp);
    else
      std::sort(d.data(), d.end(), cmp);
    assert(isSorted(d.data(), size, cmp));
  } else
    radixSort(d.data(), d.end(), rank);
}
