...
inline int l2i(const uint32& lit) {
  CHECKLIT(lit);
  return SIGN(lit) ? -int(ABS(lit)) : int(ABS(lit));
}
inline uint32 maxInactive() { return inf.maxMelted + inf.maxFrozen + inf.maxSubstituted; }
inline uint32 maxActive() {
  assert(inf.maxVar >= maxInactive());
  return inf.maxVar - maxInactive();
}
