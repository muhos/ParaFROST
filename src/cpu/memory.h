...
public:
  ...
  inline void dealloc() {
    if (_mem != NULL) std::free(_mem), _mem = NULL;
    sz = cap = 0, _junk = 0;
  }
  inline size_t bucket() const {
    assert(_bucket);
    return _bucket;
  }
  inline S size() const { return sz; }
  inline S garbage() const { return _junk; }
  inline T& operator[](const S& idx) {
    assert(check(idx));
    return _mem[idx];
  }
  inline const T& operator[](const S& idx) const {
    assert(check(idx));
    return _mem[idx];
  }
  inline T* address(const S& idx) {
    assert(check(idx));
    return _mem + idx;
  }
  inline const T* address(const S& idx) const {
    assert(check(idx));
    return _mem + idx;
  }
...
public:
  ...
  inline bool deleted(const C_REF& r) const {
    assert(check(r));
    return stencil[r];
  }
  inline void collectClause(const C_REF& r, const int& size) {
    CTYPE::collect(bytes(size));
    assert(check(r));
    stencil[r] = true;
  }
