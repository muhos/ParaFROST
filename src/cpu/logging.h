...
#define PFLOG0(MESSAGE)     \
  do {                      \
    PRINT(PREFIX);          \
    PRINT("%s\n", MESSAGE); \
  } while (0)

#define PFLOGN0(MESSAGE)  \
  do {                    \
    PRINT(PREFIX);        \
    PRINT("%s", MESSAGE); \
  } while (0)
...
#define PFLDONE(VERBOSITY, MAXVERBOSITY) \
  if (verbose >= VERBOSITY && verbose < MAXVERBOSITY) PRINT("done.\n");
...
#define PFLTRAIL(SOLVER, VERBOSITY) \
  if (verbose >= VERBOSITY) SOLVER->printTrail();

#define PFLLEARNT(SOLVER, VERBOSITY) \
  if (verbose >= VERBOSITY) SOLVER->printLearnt();

#define PFLSORTED(SOLVER, SIZE, VERBOSITY) \
  if (verbose >= VERBOSITY) SOLVER->printSortedStack(SIZE);
