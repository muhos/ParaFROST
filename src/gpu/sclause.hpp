...
class SCLAUSE {
  ...
  public:
    _PFROST_H_D_ S_REF blockSize() const {
        assert(_sz);
        return _sz + (sizeof(*this) / sizeof(uint32));
    }
    _PFROST_H_D_ size_t capacity() const {
        assert(_sz);
        return size_t(_sz) * sizeof(uint32) + sizeof(*this);
    }
    _PFROST_H_D_ SCLAUSE() : _st(ORIGINAL), _f(0), _a(0), _u(0), _lbd(0), _sig(0), _sz(0) {}
    _PFROST_H_D_ SCLAUSE(uint32* lits, const int& size) : _st(ORIGINAL), _f(0), _a(0), _u(0), _lbd(0), _sig(0), _sz(size) {
        copyLitsFrom(lits);
    }
    _PFROST_H_ SCLAUSE(CLAUSE& src) : _st(src.learnt()), _f(0), _a(0), _sig(0), _sz(src.size()) {
    ...
    _PFROST_H_D_ SCLAUSE(SCLAUSE& src) : _st(src.status()), _f(src.molten()), _a(src.added()), _u(src.usage()), _lbd(src.lbd()), _sig(src.sig()), _sz(src.size()) {
        assert(_lbd <= MAX_LBD);
        assert(!src.deleted());
        copyLitsFrom(src);
    }
    ...
    _PFROST_H_D_ void set_lbd(const unsigned& lbd) {
        assert(_lbd < MAX_LBD);
        _lbd = lbd;
    }
    ...
    _PFROST_H_D_ uint32& operator[](const int& i) {
        assert(i < _sz);
        return _lits[i];
    }
    _PFROST_H_D_ uint32 operator[](const int& i) const {
        assert(i < _sz);
        return _lits[i];
    }
    _PFROST_H_D_ uint32* data(const int& i = 0) {
        assert(i < _sz);
        return _lits + i;
    }
    _PFROST_H_D_ uint32* end() { return _lits + _sz; }
    _PFROST_H_D_ uint32 back() {
        assert(_sz);
        return _lits[_sz - 1];
    }
    _PFROST_H_D_ uint32 back() const {
        assert(_sz);
        return _lits[_sz - 1];
    }
    _PFROST_H_D_ operator uint32*() {
        assert(_sz);
        return _lits;
    }
    ...
    _PFROST_H_D_ bool has(const uint32& lit) const { // binary search
        ...
        if (_sz == 2) {
            ...
            while (first <= lit && last >= lit) {
                ...
                uint32 m = _lits[mid];
                if (m < lit)
                    first = _lits[low = mid + 1];
                else if (m > lit)
                    last = _lits[high = mid - 1];
                else
                    return true; // found
            }
            if (_lits[low] == lit)
