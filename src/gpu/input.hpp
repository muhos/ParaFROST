...
class INT_OPT : public ARG {
  ...
  public:
    ...
    virtual void help(bool verbose = false) {
        ...
        if (r.h == INT32_MIN)
            fprintf(stdout, "%-8s", "-I32");
        else
            fprintf(stdout, "%-8d", r.h);
        fprintf(stdout, " .. ");
        if (r.t == INT32_MAX)
            fprintf(stdout, "%8s", "+I32");
        else
            fprintf(stdout, "%8d", r.t);
        if (val == INT32_MAX)
            fprintf(stdout, "]%s (%sdefault: %s%10s%s)\n", CNORMAL, CARGDEFAULT, CARGVALUE, "+I32", CNORMAL);
        else
...
class INT64_OPT : public ARG {
  ...
  public:
    ...
    virtual void help(bool verbose = false) {
        PFLOGN1("  %s--%-20s = %-8s [", CHELP, arg, type);
        if (r.h == INT64_MIN)
            fprintf(stdout, "%-8s", "-I64");
        else
            fprintf(stdout, "%-8lld", r.h);
        fprintf(stdout, " .. ");
        if (r.t == INT64_MAX)
            fprintf(stdout, "%8s", "+I64");
        else
            fprintf(stdout, "%8lld", r.t);
        if (val == INT64_MAX)
...
class DOUBLE_OPT : public ARG {
  ...
  public:
    ...
    virtual void help(bool verbose = false) {
        PFLOGN1("  %s--%-20s = %-8s [", CHELP, arg, type);
        if (r.h == -INFINITY)
            fprintf(stdout, "%-8s", "-inf");
        else
            fprintf(stdout, "%-8.2f", r.h);
        fprintf(stdout, " .. ");
        if (r.t == INFINITY)
            fprintf(stdout, "%8s", "inf");
        else
            fprintf(stdout, "%8.2f", r.t);
...
class BOOL_OPT : public ARG {
  ...
  public:
    ...
    virtual void help(bool verbose = false) {
        ...
        fprintf(stdout, "                 ");
        if (val)
            fprintf(stdout, "(%sdefault: %s%3s%s)\n", CARGDEFAULT, CARGON, "on", CNORMAL);
        else
    ...
    virtual void printArgument() {
        ...
        if (val)
            fprintf(stdout, "%son ", CARGON);
        else
            fprintf(stdout, "%soff ", CARGOFF);
