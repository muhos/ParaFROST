...
_PFROST_D_ void devSort(T* data, const S& size, CMP cmp) {
    ...
    assert(data != NULL);
    if (size == 2)
        cswap(data[0], data[1], cmp);
    else if (size == 3)
        sort3(data[0], data[1], data[2], cmp);
    else if (size == 4)
        sort4(data, cmp);
    else if (size == 5)
        sort5(data, cmp);
        ...
        devInsertionSort(data, size, cmp);
...
template <typename T>
_PFROST_IN_D_ void devSwap(T& a, T& b) {
    T c = a;
    a = b, b = c;
}
...
_PFROST_IN_D_ void loadShared(T1* smem1, const T1& val1, T2* smem2, const T2& val2, const S& size) {
    if (threadIdx.x < size) {
        smem1[threadIdx.x] = val1, smem2[threadIdx.x] = val2;
    } else {
        smem1[threadIdx.x] = 0, smem2[threadIdx.x] = 0;
    }
    __syncthreads();
}
