...
class cuVec {
  ...
  public:
    ...
    _PFROST_H_D_ const T& operator[](const uint32& idx) const {
        assert(idx < sz);
        return _mem[idx];
    }
    _PFROST_H_D_ T& operator[](const uint32& idx) {
        assert(idx < sz);
        return _mem[idx];
    }
    ...
    _PFROST_H_D_ uint32* sizeptr() { return (uint32*)&sz; }
    ...
    _PFROST_H_D_ void resize(const uint32& n) {
        assert(n <= cap);
        sz = n;
    }
    ...
    _PFROST_H_D_ void print(const bool& litType = false, const bool& clauseType = false) {
        ...
        if (litType) {
            for (uint32 i = 0; i < sz; i++) {
                ...
                if (clauseType && _mem[i] < 2)
                    printf("\nc \t\t");
                else if (!clauseType && i && i < sz - 1 && i % 10 == 0)
                    printf("\nc \t\t");
            }
        } else {
            for (uint32 i = 0; i < sz; i++) {
