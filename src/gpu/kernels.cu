...
__global__ void cnt_proof_verify(const uint32* __restrict__ literals, const uint32 numLits) {
    ...
    while (tid < numLits) {
        ...
        if (lit & 0xF0000000)
            lbyte[lit] = 5;
        else if (lit & 0x0FE00000)
            lbyte[lit] = 4;
        else if (lit & 0x001FC000)
            lbyte[lit] = 3;
        else if (lit & 0x00003F80)
            lbyte[lit] = 2;
        else
            lbyte[lit] = 1;
    ...
    while (tid < pVars->size()) {
        ...
        // Equiv/NOT-gate Reasoning
            ...
            if (nClsBefore > 2) {
                ...
                if (nOrgs < SH_MAX_BVE_OUT1 && find_ao_gate(n, nClsBefore, cnf, ot, &outs[threadIdx.x * SH_MAX_BVE_OUT1], nElements, nAddedCls, nAddedLits))
                    elimType = AOIX_MASK;
                else if (!nAddedCls && pOrgs < SH_MAX_BVE_OUT1 && find_ao_gate(p, nClsBefore, cnf, ot, &outs[threadIdx.x * SH_MAX_BVE_OUT1], nElements, nAddedCls, nAddedLits))
                    elimType = AOIX_MASK;
            }
            if (!elimType && nClsBefore > 3) {
                ...
                // XOR-gate Reasoning
                    ...
                    elimType = AOIX_MASK;
                else if (!nAddedCls && find_xor_gate(n, nClsBefore, cnf, ot, &outs[threadIdx.x * SH_MAX_BVE_OUT1], nElements, nAddedCls, nAddedLits))
                    elimType = AOIX_MASK;
            }
            // fun-tab reasoning
            if (varcore && !elimType && nClsBefore > 2 && find_fun_gate(p, nClsBefore, varcore, cnf, ot, &outs[threadIdx.x * SH_MAX_BVE_OUT1], nElements, nAddedCls, nAddedLits))
                elimType = CORE_MASK;
            // n-by-m resolution
            else if (!elimType && !nAddedCls && resolve(x, nClsBefore, cnf, poss, negs, nElements, nAddedCls, nAddedLits))
                elimType = RES_MASK;
            //=====================
            // check addibility
            //=====================
...
#if VE_DBG
    if (in)
        in_ve_k_1<<<1, 1, smemSize>>>(cnf, ot, vars->pVars, vars->eliminated, vars->units, vars->resolved, proof, vars->varcore, ucnt, type, rpos, rref);
    else
        ve_k_1<<<1, 1, smemSize>>>(cnf, ot, vars->pVars, vars->eliminated, vars->units, vars->resolved, proof, vars->varcore, ucnt, type, rpos, rref);
#else
    if (in)
        in_ve_k_1<<<nBlocks, nThreads, smemSize>>>(cnf, ot, vars->pVars, vars->eliminated, vars->units, vars->resolved, proof, vars->varcore, ucnt, type, rpos, rref);
    else
        ve_k_1<<<nBlocks, nThreads, smemSize>>>(cnf, ot, vars->pVars, vars->eliminated, vars->units, vars->resolved, proof, vars->varcore, ucnt, type, rpos, rref);
#endif
