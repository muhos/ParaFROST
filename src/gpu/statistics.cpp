	...
	if (opts.report_en) {
		...
		if (opts.sigma_en || opts.sigma_live_en) {
			...
			if (!gopts.profile_gpu)
				...
				PFLOG1(" %s - CNF prepare         : %s%-16.2f  ms%s", CREPORT, CREPORTVAL, cutimer->sig, CNORMAL);
				...
				PFLOG1(" %s - SUB                 : %s%-16.2f  ms%s", CREPORT, CREPORTVAL, cutimer->sub, CNORMAL);
			...
			if (opts.proof_en) {
				PFLOG1(" %s Proof lines           : %s%-16zd%s", CREPORT, CREPORTVAL, cuproof.gpuClauses(), CNORMAL);
				PFLOG1(" %s Proof bytes           : %s%-16.3f  MB%s", CREPORT, CREPORTVAL, (double)cuproof.gpuBytes() / MBYTE, CNORMAL);
			}
		}
		...
		if (opts.proof_en) {
			PFLOG1(" %s Proof lines           : %s%-16zd%s", CREPORT, CREPORTVAL, proof.clauses(), CNORMAL);
			PFLOG1(" %s Proof bytes           : %s%-16.3f  MB%s", CREPORT, CREPORTVAL, (double)proof.bytes() / MBYTE, CNORMAL);
		}
