    ...
    if (vars->nUnits) {
        ...
        for (uint32* u = vars->cachedUnits; u != t; u++) {
            ...
            if (UNASSIGNED(val))
                ...
                return false; // early conflict detection
        }
        if (trail.size() == sp->propagated)
            vars->nUnits = nForced = 0; // duplicate units
        else
            PFLOGN2(2, " Propagating pending forced units..");
...
inline bool Solver::enqueueCached(const cudaStream_t& stream) {
    if (vars->nUnits) {
        ...
        if (trail.size() == sp->propagated)
            ...
            PFLOGN2(2, " Propagating forced units..");
