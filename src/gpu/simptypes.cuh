...
struct VARS {
    ...
    VARS() : pVars(NULL), units(NULL), resolved(NULL), pVarsData(NULL), pVarsSize(NULL), scores(NULL), varcore(NULL), eligible(NULL), cachedUnits(NULL), eliminated(NULL), cachedEliminated(NULL), numPVs(0), nUnits(0), nMelted(0), isEliminatedCached(false) {}
};
struct cuHist {
    S_REF* d_segs;
    ...
    t_iptr thrust_hist;
    cuHist() : d_segs(NULL), d_hist(NULL), h_hist(NULL), d_vorg(NULL), d_lbyte(NULL) {}
    ~cuHist() {
...
class CNF {
  ...
  public:
    _PFROST_H_D_ CNF() : _data({ NULL, 0, 0 }), _bucket((Byte)sizeof(uint32)) {}
    ...
    _PFROST_H_D_ S_REF ref(const uint32& i) {
        assert(i < _refs.size());
        return _refs[i];
    }
    _PFROST_H_D_ const S_REF& ref(const uint32& i) const {
        assert(i < _refs.size());
        return _refs[i];
    }
    _PFROST_H_D_ SCLAUSE& clause(const uint32& i) {
        assert(ref(i) < _data.size);
        return (SCLAUSE&)_data.mem[ref(i)];
    }
    _PFROST_H_D_ const SCLAUSE& clause(const uint32& i) const {
        assert(ref(i) < _data.size);
        return (SCLAUSE&)_data.mem[ref(i)];
    }
    ...
    _PFROST_H_D_ SCLAUSE& operator[](const S_REF& r) {
        assert(r < _data.size);
        return (SCLAUSE&)_data.mem[r];
    }
    _PFROST_H_D_ const SCLAUSE& operator[](const S_REF& r) const {
        assert(r < _data.size);
        return (SCLAUSE&)_data.mem[r];
    }
    ...
    _PFROST_H_D_ void print(const uint32& off = 0, const bool& p_ref = true) {
        for (uint32 i = off; i < size(); i++) {
            ...
            if (c.size()) {
                if (p_ref)
                    printf("c  C(%d, r: %lld)->", i, uint64(_refs[i]));
                else
                    printf("c  C(%d)->", i);
                c.print();
            }
...
class OT {
  ...
  public:
    ...
    _PFROST_H_D_ OL& operator[](const uint32& i) {
        assert(i < maxLists);
        return _lists[i];
    }
    _PFROST_H_D_ OL operator[](const uint32& i) const {
        assert(i < maxLists);
        return _lists[i];
    }
