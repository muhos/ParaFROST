...
inline bool Solver::reallocCNF() {
    ...
    if (times > 1 && times != opts.phases && (times % opts.shrink_rate) == 0) {
        ...
        cumm.cacheCNFPtr(cnf), compacted = false;
    return true;
}

inline bool Solver::reallocOT(const cudaStream_t& stream) {
    assert(inf.nLiterals);
    if (!flattenCNF(inf.nLiterals)) {
        simpstate = OTALLOC_FAIL;
        return false;
    }
    histSimp(inf.nLiterals);
    if (!cumm.resizeOTAsync(ot, inf.nLiterals, stream)) {
        simpstate = OTALLOC_FAIL;
        return false;
    }
    return true;
...
void Solver::awaken() {
    ...
    if (!cumm.allocHist(cuhist, opts.proof_en) || !cumm.allocAux(numCls) || !cumm.allocVars(vars, savedLits) || !cumm.allocPinned(vars, cuhist) || !cumm.resizeCNF(cnf, numCls, numLits)) {
