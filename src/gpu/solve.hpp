...
class Solver {
  ...
  public:
    ...
    inline bool varsEnough() const {
        assert(trail.size() < inf.maxVar);
        return (inf.maxVar - trail.size()) > last.mdm.unassigned;
    }
    ...
    inline void initQueue() {
        if (verbose == 4)
            PFLOG2(2, " Initializing VMFQ Queue with %d variables..", inf.maxVar);
        else
            PFLOGN2(2, " Initializing VMFQ Queue with %d variables..", inf.maxVar);
    ...
    inline void markLit(const uint32& lit) {
        CHECKLIT(lit);
        sp->marks[ABS(lit)] = SIGN(lit);
    }
    inline void unmarkLit(const uint32& lit) {
        CHECKLIT(lit);
        sp->marks[ABS(lit)] = UNDEFINED;
    }
    inline void markProbe(const uint32& lit) {
        CHECKLIT(lit);
        sp->vstate[ABS(lit)].probe = 1;
    }
    inline void unmarkProbe(const uint32& lit) {
        CHECKLIT(lit);
        sp->vstate[ABS(lit)].probe = 0;
    }
    inline void markSubsume(const uint32& lit) {
        CHECKLIT(lit);
        sp->vstate[ABS(lit)].subsume = 1;
    }
    inline void unmarkSubsume(const uint32& lit) {
        CHECKLIT(lit);
        sp->vstate[ABS(lit)].subsume = 0;
    }
    inline bool markedSubsume(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->vstate[ABS(lit)].subsume;
    }
    inline bool markedProbe(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->vstate[ABS(lit)].probe;
    }
    inline bool active(const uint32& lit) const {
        CHECKLIT(lit);
        return !sp->vstate[ABS(lit)].state;
    }
    inline bool inactive(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->vstate[ABS(lit)].state;
    }
    inline bool subsumed(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->marks[ABS(lit)] == SIGN(lit);
    }
    inline bool notsubsumed(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->marks[ABS(lit)] ^ SIGN(lit);
    }
    inline bool selfsubsumed(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->marks[ABS(lit)] == !SIGN(lit);
    }
    inline LIT_ST l2marker(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->marks[ABS(lit)];
    }
    inline int l2dl(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->level[ABS(lit)];
    }
    inline C_REF l2r(const uint32& lit) const {
        CHECKLIT(lit);
        return sp->source[ABS(lit)];
    }
    inline LIT_ST unassigned(const uint32& lit) const {
        CHECKLIT(lit);
        return UNASSIGNED(sp->value[lit]);
    }
    inline LIT_ST isFalse(const uint32& lit) const {
        CHECKLIT(lit);
        return !sp->value[lit];
    }
