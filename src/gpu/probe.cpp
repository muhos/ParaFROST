...
struct PROBE_QUEUE_CMP {
    ...
    PROBE_QUEUE_CMP(const VSTATE* _states, const Vec<uint64>& _bumped) : states(_states), bumped(_bumped) {}
...
struct PROBE_HEAP_CMP {
    ...
    PROBE_HEAP_CMP(const VSTATE* _states, const Vec<double>& _act) : states(_states), act(_act) {}
...
void Solver::FLE() {
    ...
    for (int round = 1; round <= opts.probe_min; round++) {
        ...
        if (probes.size()) {
            if (stable)
                Sort(probes, PROBE_HEAP_CMP(states, activity));
            else
                Sort(probes, PROBE_QUEUE_CMP(states, bumps));
        } else {
...
void Solver::probe() {
    ...
    assert(removed >= 0);
    if (removed)
        PFLOG2(2, " Probe call %lld removed %d variables %.2f%%", stats.probe.calls, removed, percent(removed, before));
    else
        PFLOG2(2, " Probe %lld: did not remove any variables", stats.probe.calls);
