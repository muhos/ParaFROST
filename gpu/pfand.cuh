...
#pragma unroll
		...
		_PFROST_D_ bool find_ao_gate(const uint32& dx, const uint32& nOrgCls, CNF& cnf, OT& ot, uint32* out_c, uint32& nAddedCls, uint32& nAddedLits)
		{
			assert(dx > 1);
			...
			if (nImps > 1) {
				...
				forall_occurs(otarget, i) {
					...
					if (c.size() == nImps && sub(c.sig(), sig) && isEqual(c, out_c, nImps)) {
						...
						if (countSubstituted(x, nOrgCls, cnf, itarget, otarget, nAddedCls, nAddedLits)) { c.freeze(); break; }
						// can be substituted
#if VE_DBG
