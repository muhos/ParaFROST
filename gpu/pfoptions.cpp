...
BOOL_OPT opt_atomic_ve_en("atomicve", "enable atomic version of BVE (produces outoforder resolvents)", false);
BOOL_OPT opt_aggr_cnf_sort("aggressivesort", "sort simplified formula with aggresive key before writing to host", false);
BOOL_OPT opt_gc_par_en("gcpar", "enable device garbage collection in parallel (stream compaction approach)", true);
...
DOUBLE_OPT opt_lits_mul("literalsmul", "initial literals multiplier", 1.2, FP64R(1, 2));
