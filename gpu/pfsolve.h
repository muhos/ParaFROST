...
namespace pFROST {
	...
	public:
		...
		inline uint32   recoverLit          (const uint32& mlit) const {
			CHECKLIT(mlit); 
			return V2DEC(vorg[ABS(mlit)], SIGN(mlit));
		}
	...
	public:
		...
		void			iassume				(Lits_t&);
		void			isolve				(Lits_t&);
		inline void		ifreeze				(const uint32& v) { CHECKVAR(v); ifrozen[v] = 1; }
		inline void		iunfreeze			(const uint32& v) { CHECKVAR(v); ifrozen[v] = 0; }
		inline bool		iassumed			(const uint32& v) const { CHECKVAR(v); return incremental && ifrozen[v]; }
		inline bool		ieliminated			(const uint32& v) const {
			CHECKVAR(v); 
			return MELTED(sp->vstate[v].state) || SUBSTITUTED(sp->vstate[v].state);
		}
		//==========================================//
		//			       Printers                 //
		//==========================================//
