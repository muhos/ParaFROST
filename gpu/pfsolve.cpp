...
void ParaFROST::report()
{
	if (opts.report_en) {
		...
		if (opts.profile_simp) {
			...
			PFLOG1("  - ERE                 : %-10.2f  ms", cutimer->ere);
		}
		PFLOG1(" Device memory          : %-10.3f  MB", ((double)(cumm.maxCapacity() + tca.maxCapacity()) / MBYTE));
		...
		PFLOG1(" ERE redundancies       : %-10lld", stats.n_reduns);
